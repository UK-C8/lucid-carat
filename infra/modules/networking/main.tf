################################################################################
# Module: networking
# Creates VPC, public/private subnets, IGW, NAT gateway, and security groups.
# Two public subnets (ALB) + two private subnets (ECS tasks, RDS, Redis).
################################################################################
...
# ALB: public HTTPS + HTTP (redirect)
resource "aws_security_group" "alb" {
  name        = "${var.name_prefix}-sg-alb"
  description = "ALB — allow inbound HTTPS/HTTP from the internet"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "ecs_tasks" {
  ...
  description = "ECS Fargate tasks — ingress from ALB, egress to internet/data tier"
  vpc_id      = aws_vpc.main.id
...
# Redis: allow from ECS tasks only

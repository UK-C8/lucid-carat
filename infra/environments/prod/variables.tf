...
variable "media_bucket_name" {
  type    = string
  default = "lucidcarat-prod-media"
}

variable "cors_allowed_origins" {
  type    = list(string)
  default = ["https://app.lucidcarat.com"]
}

# ── RDS (prod: larger instance, multi-AZ, deletion protection on) ─────────────
...
variable "db_password" {
  description = "RDS master password — provide via TF_VAR_db_password. Never hardcode."
  type        = string
  sensitive   = true
}

# ── Redis (prod: with replica) ─────────────────────────────────────────────────
